...
var EJ = (() => {
  ...
  function generateOrderReceiptTexts(detail, config, opts) {
    ...
    const refundTimeStr = detail.refundRecord?.refundTime ? String(detail.refundRecord.refundTime) : orderTimeStr;
    ...
    if (detail.refundRecord) {
      const returnItems = ["Cashier Copy", "Customer Copy"].map((copyType) => ({
        text: generateReceipt(
          "RETURN_TXN" /* RETURN_TXN */,
          mapOrderToReturnReceiptData(detail, headerConfig, { ...opts, copyType }),
          headerConfig
        ),
        time: refundTimeStr
      }));
  ...
  function generateShiftReceiptText(shift, config, opts) {
    return generateReceipt(
      "X_READING" /* X_READING */,
      mapShiftToXReadingData(shift, config, opts),
      config
    );
  }
  ...
  function generateDailySettlementReceiptText(ds, config, opts) {
    const data = mapDailySettlementToZReadingData(ds, config, opts);
